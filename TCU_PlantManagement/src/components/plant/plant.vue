...
export default {
  ...
  methods: {
    ...
    // 初始化数据
    async init() {
      let lists = await apis.list.findAndCountAllByKind(
        this.kind,
        this.off,
        this.PageShowSum
      );
    ...
    async toCreat() {
      ...
      const { tid, card, x, y, height, lon, lat, name, kind_id, plant_id } =
        this.createItem;
      if (
        !tid ||
        !card ||
    ...
    async getSearch() {
      let search = this.doSearchText;
      let lists = await apis.list.findAndCountAllByName(
        search,
        this.off,
        this.PageShowSum
      );
