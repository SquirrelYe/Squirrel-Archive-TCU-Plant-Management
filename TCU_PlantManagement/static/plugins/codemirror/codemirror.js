...
window.CodeMirror = (function () {
  ...
  var ie_lt8 =
    old_ie && (document.documentMode == null || document.documentMode < 8);
  var ie_lt9 =
    old_ie && (document.documentMode == null || document.documentMode < 9);
  var ie_lt10 =
    old_ie && (document.documentMode == null || document.documentMode < 10);
  ...
  function measureLineInner(cm, line) {
    ...
    for (var i = 0, cur; i < measure.length; ++i)
      if ((cur = measure[i])) {
        ...
        if (cur.measureRight)
          rect.right = getRect(cur.measureRight).left - outer.left;
  ...
  function readInput(cm) {
    ...
    if (same < prevInput.length)
      ...
      to = Pos(
        to.line,
        Math.min(getLine(doc, to.line).text.length, to.ch + inserted.length)
      );
  ...
  CodeMirror.prototype = {
    ...
    indentSelection: operation(null, function (how) {
      ...
      if (posEq(sel.from, sel.to))
        return indentLine(this, sel.from.line, how, true);
  ...
  function markText(doc, from, to, options, type) {
    ...
    if (
      posLess(to, from) ||
      (posEq(from, to) && marker.clearWhenEmpty !== false)
    )
      return marker;
