...
module.exports = {
  ...
  findAndCountAll(req, res) {
    list.findAndCountAll().then((msg) => {
      res.send(msg);
    });
  },
  // 新建信息
  create(req, res) {
    list.create(req.body).then((msg) => {
      res.send(msg);
    });
  },
  // 删除信息
  delete(req, res) {
    const { id } = req.body;
    list.destroy({ where: { id } }).then((msg) => {
      res.send({ del: msg });
    });
  },
  // 更新信息
  ...
  findByRand(req, res) {
    ...
    conn
      .query(`select lon,lat from lists order by rand() limit ${number};`)
      .then((msg) => {
        res.send(msg);
      });
  },
        ...
        for (let item of msg) {
          ...
          code.pipe(
            fs.createWriteStream(
              `./card/${item.dataValues.id}_${item.dataValues.card}.png`
            )
          );
        }
        res.send(msg);
