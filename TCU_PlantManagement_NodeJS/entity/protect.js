...
// 模型层定义
let protect = conn.define("protect", {
  id: {
...
module.exports = {
  ...
  findAndCountAll(req, res) {
    protect.findAndCountAll().then((msg) => {
      res.send(msg);
    });
  },
  // 新建信息
  create(req, res) {
    protect.create(req.body).then((msg) => {
      res.send(msg);
    });
  },
  // 删除信息
  delete(req, res) {
    const { id } = req.body;
    protect.destroy({ where: { id } }).then((msg) => {
      res.send({ del: msg });
    });
  },
  // 更新信息
