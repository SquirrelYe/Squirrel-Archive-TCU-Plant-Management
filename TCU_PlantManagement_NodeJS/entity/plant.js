...
module.exports = {
  ...
  findAndCountAll(req, res) {
    plant.findAndCountAll().then((msg) => {
      res.send(msg);
    });
  },
  // 按id查询
  ...
  // 创建信息
  create(req, res) {
    plant.create(req.body).then((msg) => res.send(msg));
  },
  // 删除信息
  delete(req, res) {
    const { id } = req.body;
    plant.destroy({ where: { id } }).then((msg) => {
      res.send({ del: msg });
    });
  },
  // 更新信息
  update(req, res) {
    const { id } = req.body;
    plant.update(req.body, { where: { id } }).then((msg) => {
      res.send({ upd: msg[0] });
    });
  },
  // 获取数量
  count(req, res) {
    plant.count().then((msg) => {
      res.send({ number: msg });
    });
  },
};
